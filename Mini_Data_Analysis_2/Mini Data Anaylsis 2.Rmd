...
head(cancer_sample)
```
This dataset, is by the above standards, a tidy data set. Each observation (ID) is a new row and each variable (diagnosis, radius_mean, texture_mean etc.) all occupy their own column. Additionally, the variable names are an acceptable format that doesn't present issues for mistyping (capitals etc). 
...
```{r}
size_diagnosis_model<- glm(diagnosis ~ radius_mean + smoothness_mean, data = cancer_sample, family = binomial)
